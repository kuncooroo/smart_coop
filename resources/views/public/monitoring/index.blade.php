...
@section('content')
    <div class="mb-10 flex justify-between items-center">
        <div>
            <h2 class="text-3xl font-extrabold text-slate-800 tracking-tight">Monitoring Real-time</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
        @forelse($kandangs as $k)
            ...
            <div class="bg-white rounded-[2rem] shadow-sm overflow-hidden border border-slate-100 group hover:shadow-xl transition-all duration-300">
                <div class="h-44 relative overflow-hidden">
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-[#002855]/90 via-transparent flex flex-col justify-end p-6">
                ...
                <div class="p-6 space-y-5">
                    <div class="grid grid-cols-2 gap-3">
                        <div class="bg-slate-50 p-3 rounded-xl border border-slate-100">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="text-sm font-extrabold text-slate-800">{{ $k->capacity }} <small class="text-[9px] text-slate-400">EKOR</small></span>
                            </div>
                        </div>
                        <div class="bg-slate-50 p-3 rounded-xl border border-slate-100">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="text-sm font-extrabold text-slate-800">{{ $latestSensor->temperature ?? '0' }}°C</span>
                            </div>
                        </div>
                    </div>

                    <div class="p-4 rounded-2xl flex items-center justify-between {{ $servo && $servo->door_status == 'TERBUKA' ? 'bg-emerald-50 border border-emerald-100' : 'bg-[#002855] border border-[#002855]' }} transition-all">
                        <div class="flex items-center space-x-3">
                            <div class="w-9 h-9 flex items-center justify-center rounded-xl {{ $servo && $servo->door_status == 'TERBUKA' ? 'bg-emerald-500 text-white' : 'bg-slate-700 text-slate-300' }}">
                                <i class="fas {{ $servo && $servo->door_status == 'TERBUKA' ? 'fa-door-open' : 'fa-door-closed' }} text-sm"></i>
                            </div>
                            <div>
                                <p class="text-[8px] uppercase font-black {{ $servo && $servo->door_status == 'TERBUKA' ? 'text-emerald-600' : 'text-slate-400' }}">Pintu Utama</p>
                                <p class="font-black tracking-widest text-[11px] {{ $servo && $servo->door_status == 'TERBUKA' ? 'text-emerald-700' : 'text-white' }}">{{ $servo->door_status ?? 'OFFLINE' }}</p>
                            </div>
                        </div>
                        @if ($servo)
                            <form action="{{ route('commands.store') }}" method="POST" class="m-0">
                                ...
                                <input type="hidden" name="command" value="{{ $servo->door_status == 'TERBUKA' ? 'CLOSE_DOOR' : 'OPEN_DOOR' }}">
                                <button type="submit"
                    ...
                    <div class="p-4 rounded-2xl bg-white flex items-center justify-between border border-slate-100 {{ $lamp && $lamp->light_status == 'HIDUP' ? 'ring-1 ring-orange-500 bg-orange-50/30' : '' }}">
                        <div class="flex items-center space-x-3">
                            <div class="w-9 h-9 flex items-center justify-center rounded-xl {{ $lamp && $lamp->light_status == 'HIDUP' ? 'bg-orange-500 text-white' : 'bg-slate-100 text-slate-400' }}">
                                <i class="fas fa-lightbulb text-sm"></i>
                            </div>
                            <div>
                                <p class="text-[8px] uppercase font-black text-slate-400 tracking-widest">Pemanas</p>
                                <p class="font-black text-slate-800 text-[11px] tracking-widest">{{ $lamp->light_status ?? 'OFFLINE' }}</p>
                            </div>
                        </div>
                        @if ($lamp)
                            <form action="{{ route('commands.store') }}" method="POST" class="m-0">
                                ...
                                <input type="hidden" name="command" value="{{ $lamp->light_status == 'HIDUP' ? 'LIGHT_OFF' : 'LIGHT_ON' }}">
                                <button type="submit"
                                    ...
                                    <span class="inline-block h-4 w-4 transform rounded-full bg-white transition duration-200 {{ $lamp->light_status == 'HIDUP' ? 'translate-x-5' : 'translate-x-1' }}"></span>
                                </button>
                            </form>
                        @endif
                    </div>
                        ...
                        @if ($k->setting)
                            <div>
                                <h5 class="text-[9px] font-black text-emerald-600 uppercase tracking-[0.1em] mb-1">
                                    <i class="fas fa-clock mr-1"></i> Auto-Schedule
                                </h5>
                                <div class="flex items-center gap-2">
                                    <span class="text-xs font-extrabold text-slate-700">{{ \Carbon\Carbon::parse($k->setting->timer_open)->format('H:i') }}</span>
                                    <span class="text-slate-300 text-xs">-</span>
                                    <span class="text-xs font-extrabold text-slate-700">{{ \Carbon\Carbon::parse($k->setting->timer_close)->format('H:i') }}</span>
                                </div>
                            </div>
                        @else
                            <div>
                                <h5 class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Penjadwalan</h5>
                                <p class="text-[10px] text-slate-400 font-bold uppercase italic">Manual Mode</p>
                            </div>
                        @endif
                        <button type="button" onclick="openModal('modal-{{ $k->id }}')"
                            class="w-9 h-9 rounded-lg bg-slate-50 border border-slate-100 text-slate-400 hover:text-[#002855] hover:bg-orange-50 hover:border-orange-200 transition-all flex items-center justify-center cursor-pointer">
            ...
            <div id="modal-{{ $k->id }}"
                ...
                <div class="bg-white rounded-[2rem] w-full max-w-sm shadow-2xl overflow-hidden border border-slate-200">
                    <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        <h3 class="text-sm font-black text-[#002855] uppercase tracking-widest">Settings: {{ $k->name }}</h3>
                        <button type="button" onclick="closeModal('modal-{{ $k->id }}')"
                            class="text-slate-400 hover:text-rose-500 transition-colors"><i class="fas fa-times"></i></button>
                    </div>
                    <form action="{{ route('settings.update', $k->id) }}" method="POST" class="p-6 space-y-5">
                        @csrf @method('PUT')
                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-2">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Buka Pintu</label>
                                <input type="time" name="timer_open" value="{{ $k->setting->timer_open ?? '06:00' }}"
                            ...
                            <div class="space-y-2">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tutup Pintu</label>
                                <input type="time" name="timer_close"
                                    value="{{ $k->setting->timer_close ?? '18:00' }}"
                                    class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 focus:ring-2 focus:ring-orange-500 outline-none">
                            </div>
                        </div>
                        <button type="submit"
        ...
        @empty
            <div class="col-span-full bg-white p-16 rounded-[2rem] border border-dashed border-slate-200 text-center">
                ...
                <p class="text-slate-400 font-bold uppercase text-xs tracking-widest">No poultry unit detected</p>
            </div>
        @endforelse
    </div>
@endsection